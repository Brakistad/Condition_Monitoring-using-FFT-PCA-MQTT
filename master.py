...
while(i<1000):
    ...
    if(works):
        ...
        if (not (fft_worker.isAlive()) and not (pca_worker.isAlive()) and (datacount<pca_object_amount)):
            fft_worker = threading.Thread(target = analyse_FFT);
            fft_worker.start();
        if (not (pca_worker.isAlive()) and not (fft_worker.isAlive()) and (datacount>=pca_object_amount)):
            pca_worker = threading.Thread(target = analyse_pca);
            pca_worker.start();
    else:
